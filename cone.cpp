...
namespace static_meshes_3D {
	...
	Cone::Cone(float radiusTop, float radiusBottom, int numSlices, float height, bool withPositions, bool withTextureCoordinates, bool withNormals)
	...
	float Cone::getRadiusTop() const
	{
		return _radiusTop;
	}

	float Cone::getRadiusBottom() const
	{
	...
	void Cone::initializeData()
	{
		...
		if (hasPositions())
		{
			...
			for (auto i = 0; i <= _numSlices; i++)
			{
				xT.push_back(cosines[i] * _radiusTop);
				zT.push_back(sines[i] * _radiusTop);
			}
			...
			for (auto i = 0; i <= _numSlices; i++)
			{
				xB.push_back(cosines[i] * _radiusBottom);
				zB.push_back(sines[i] * _radiusBottom);
			}

			// Add cylinder side vertices
			for (auto i = 0; i <= _numSlices; i++)
			{
				const auto topPosition = glm::vec3(xT[i], _height / 2.0f, zT[i]);
				const auto bottomPosition = glm::vec3(xB[i], -_height / 2.0f, zB[i]);
